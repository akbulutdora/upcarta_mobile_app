import 'package:flutter/material.dart';

class AppColors {
  //Main App Colors - BERNA
  //WHITES
  static Color white = const Color(0xffffffff);
  static Color gray0Line = const Color(0xffeef1f4);
  static Color gray1BoxFrame = const Color(0xffDEDEDE);
  static Color gray2LightText = const Color(0xffC4C4C4);
  static Color gray3ContentText = const Color(0xff949494);
  static Color black = Colors.black;

  //BLUES
  static Color blue0ContentBoxFill = const Color(0xfff8fbff);
  static Color blue1 = const Color(0xffd1e0fe);
  static Color blue2 = const Color(0xffa4c3fe);
  static Color upcartaBlue = const Color(0xff4e89fd);
// TODO: ADD OTHER COLORS

  static Color primary = const Color(0xff4e89fd);
  static Color secondary = Colors.black;
  static Color line = const Color(0xffEEF1F4);
  static Color boxFrame = const Color(0xffDEDEDE);
  static Color lightText = const Color(0xffc4c4c4);
  static Color contentText = const Color(0xff949494);
  static Color contentBoxFill = const Color(0xfff8fbff);
  static Color edgeColor = const Color(0xffC4C4C4);

  //Library screen colors
  static Color tabBarColor = const Color(0xffFFFFFF);
  static Color selected = const Color(0xff4E89FD);
  static Color tabBarSelectedText = const Color(0xff000000);
  static Color tabBarOtherText = const Color(0xff667080);
  static Color chip = const Color(0xffFFFFFF);

  //Light Theme Colors
  static Color lightDivider = const Color(0xff949494);

  //Dark Theme Colors
  static Color darkPrimary = Colors.deepPurpleAccent;
  static Color appBar = Colors.black45;
  static Color darkDivider = Colors.white;
}
